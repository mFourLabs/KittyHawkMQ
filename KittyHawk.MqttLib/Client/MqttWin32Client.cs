...
namespace KittyHawk.MqttLib.Client
{
    public sealed class MqttClient : IDisposable
    {
        ...
        public static MqttClient CreateClient()
        {
			...
			var client = new MqttClient(new Win32SocketAdapter(logManager), logManager, SocketEncryption.None);
        ...
        public static MqttClient CreateSecureClient(SocketEncryption encryption)
        {
            ...
            ActiveClientCollection.Instance.TestCanAddNewClient();
            // Each platform has its own implementation of Sockets and reader threads.
            var client = new MqttClient(new Win32SocketAdapter(logManager), logManager, encryption);
            ActiveClientCollection.Instance.AddClient(client);
            return client;
        }
        ...
        public Task<MqttConnectAckMessage> ConnectAsync(string clientId, string ipOrHost)
        {
        ...
        /// <returns>MqttConnectAckMessage representing the message returned from the broker</returns>
        public Task<MqttConnectAckMessage> ConnectAsync(string clientId, string ipOrHost, int port)
        {
            var bldr = new MqttConnectMessageBuilder
            {
                ClientId = clientId
        ...
        /// <param name="ipOrHost">An IP address or host name</param>
        /// <returns>MqttConnectAckMessage representing the message returned from the broker</returns>
        public Task<MqttConnectAckMessage> ConnectWithMessageAsync(MqttConnectMessageBuilder msgBuilder, string ipOrHost)
        {
            int port = IsEncrypted ? MqttProtocolInformation.Settings.SecurePort : MqttProtocolInformation.Settings.Port;
        ...
        public Task PublishStringAsync(string topic, string payload)
        {
        ...
        public Task PublishStringAsync(string topic, string payload, QualityOfService qos, int messageId, bool retain)
        {
            // Encode the string into a byte array
            byte[] utf8Str = Encoding.UTF8.GetBytes(payload);
        ...
        public Task PublishUIntAsync(string topic, UInt16 payload)
        {
        ...
        public Task PublishUIntAsync(string topic, UInt16 payload, QualityOfService qos, int messageId, bool retain)
        {
            // Encode the uint into a byte array
            int pos = 0;
            var payloadBytes = new byte[2];
        ...
        private Task SendMessageAsync(IMqttMessageBuilder msgBuilder)
        {
            var tcs = new TaskCompletionSource<object>();
