...
{
    internal class Win32SocketAdapter : ISocketAdapter
    {
        private string _remoteHost;
        private readonly ILogger _logger;
        private readonly Win32SocketWorker _socketWorker;

        public Win32SocketAdapter(ILogger logger)
        {
            _logger = logger;
            _socketWorker = new Win32SocketWorker(_logger);
        ...
        private Stream GetStream(TcpClient tcpClient, SslProtocols encryption)
        {
            if (encryption != SslProtocols.None)
            {
                ...
                var sslStream = new SslStream(tcpClient.GetStream(), false, ValidateServerCertificate, null, EncryptionPolicy.RequireEncryption);
